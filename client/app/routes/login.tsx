...
export async function clientAction({ request }: Route.ClientActionArgs) {
    ...
    try {
      const req = await fetch(serverUrl+"/users/verifyEmail", {
        method:"POST",
        headers: {
      ...
      if(response.error) return {error: "Pas de compte a cet email!"}
    } catch (error) {
      return {error}
    }
...
export default function Login() {
  ...
  useEffect(() => {
    if(user.fullName) navigate("/admin")
    const toastOptions = { duration: 5000 };
    if (fetcher.data && !fetcher.data.error) {
      toast.success("Email envoye", toastOptions);
  ...
  return (
    <>
      <div data-theme="black" className="flex h-auto min-h-screen items-center justify-center overflow-x-hidden py-10">
          ...
          <div className="bg-base-100 shadow-base-300/20 z-1 w-full space-y-6 rounded-xl p-6 shadow-md sm:min-w-md lg:p-8">
            ...
            <div>
              <h3 className="text-base-content mb-1.5 text-2xl font-semibold">
                Login to  <Link className="link text-base-content link-neutral text-xl font-bold no-underline flex" to="/">bilson <span className="text-red-500 text-xl">.</span> </Link>
              </h3>
